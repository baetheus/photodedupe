...
fn load_image_from_file(image_path: &str) -> std::result::Result<DynamicImage, MyImageError> {
    ...
    let format_guessed = match img.with_guessed_format() {
        ...
        Err(_) => {
            return Err(MyImageError::DecodeFail(
                format!("Error: Failed to identify image file format {}", image_path).to_string(),
            ));
        }
    };
    ...
    let decoded_img = match format_guessed.decode() {
        ...
        Err(_) => {
            return Err(MyImageError::DecodeFail(
                format!("Error: Failed to correctly decode image: {}", image_path).to_string(),
            ));
        }
    };
...
impl ImageHashAV {
    ...
    pub fn diff_colour(&self, comp: &ImageHashAV) -> u64 {
        ...
        for i in 0..64 {
            let rdiff: u32 =
                (comp.low_res[i * 3] as i32 - self.low_res[i * 3] as i32).abs() as u32;
            let gdiff: u32 =
                (comp.low_res[(i * 3) + 1] as i32 - self.low_res[(i * 3) + 1] as i32).abs() as u32;
            let bdiff: u32 =
                (comp.low_res[(i * 3) + 2] as i32 - self.low_res[(i * 3) + 2] as i32).abs() as u32;
    ...
    pub fn calc_std_dev_colour_hash(&mut self) {
        ...
        self.std_dev =
            (r_square_total.sqrt() + g_square_total.sqrt() + b_square_total.sqrt()) / 3.0;
    }
            ...
            Ok(img) => {
                ...
                if width < 16 || height < 16 {
                    return Some(MyImageError::ImageTooSmall(
                        format!("Warning: Image too small to deduplicate: {}", im_path.fpath)
                            .to_string(),
                    ));
                }
                ...
                match fs::metadata(im_path.fpath.clone()) {
                    ...
                    Err(_) => {
                        return Some(MyImageError::FileError(
                            format!("Error: Failed to get size of: {}", im_path.fpath).to_string(),
                        ));
                    }
                }
                ...
                if width != 8 || height != 8 {
                    return Some(MyImageError::DecodeFail(
                        format!("Error: Failed to resize image correctly: {}", im_path.fpath)
                            .to_string(),
                    ));
                }
...
mod tests {
    ...
    #[test]
    fn test_image_read() {
        let result = ImageHashAV::new(&ImagePath {
    ...
    fn test_image_duplicates() {
        ...
        for i in 0..(image_paths.len() / 3) {
            ...
            .unwrap();
            let dupe1 = ImageHashAV::new(&ImagePath {
                fpath: image_paths[(i * 3) + 1].clone(),
                is_compare_dir: false,
                always_mark_dupe_compare: false,
            })
            .unwrap();
            let dupe2 = ImageHashAV::new(&ImagePath {
            ...
            assert!(
                result.diff_colour(&dupe1) <= ImageHashAV::DEFAULT_COLOUR_DIFF_THRESHOLD,
                "First duplicate colours are similar"
            );
            assert!(
                result.diff_colour(&dupe2) <= ImageHashAV::DEFAULT_COLOUR_DIFF_THRESHOLD,
    ...
    fn test_image_uniques() {
        ...
        for i in 0..image_hashes.len() {
            for j in (i + 1)..image_hashes.len() {
                ...
                assert!(
                    (calc_hamming_distance(image_hashes[j].dupe_group, image_hashes[i].dupe_group)
                        <= 63)
